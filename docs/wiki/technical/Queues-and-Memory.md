...
This page documents the bounded queue implementation in src/queue.rs ([source](/rylan-meilutis/sedsprintf_rs/blob/main/src/queue.rs)) and how it affects router and relay behavior.
...
These values are set at compile time via src/config.rs ([source](/rylan-meilutis/sedsprintf_rs/blob/main/src/config.rs)):
...
They can be overridden using `build.py env:KEY=VALUE` (build.py: [source](/rylan-meilutis/sedsprintf_rs/blob/main/build.py)) or `.cargo/config.toml`. (build.py: [source](/rylan-meilutis/sedsprintf_rs/blob/main/build.py))
