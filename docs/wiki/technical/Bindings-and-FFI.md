...
- src/c_api.rs ([source](/rylan-meilutis/sedsprintf_rs/blob/main/src/c_api.rs)) defines the C ABI surface.
- build.rs ([source](/rylan-meilutis/sedsprintf_rs/blob/main/build.rs)) generates C-Headers/sedsprintf.h ([source](/rylan-meilutis/sedsprintf_rs/blob/main/C-Headers/sedsprintf.h)) from the schema.
...
- src/python_api.rs ([source](/rylan-meilutis/sedsprintf_rs/blob/main/src/python_api.rs)) defines the pyo3 module.
- build.rs ([source](/rylan-meilutis/sedsprintf_rs/blob/main/build.rs)) generates python-files/sedsprintf_rs/sedsprintf_rs.pyi ([source](/rylan-meilutis/sedsprintf_rs/blob/main/python-files/sedsprintf_rs/sedsprintf_rs.pyi)) for type hints.
...
Both bindings are generated from telemetry_config.json ([source](/rylan-meilutis/sedsprintf_rs/blob/main/telemetry_config.json)), plus built-in `TelemetryError` endpoint/type (which must not
