...
## C/C++ binding
- `src/c_api.rs` defines the C ABI surface.
- `build.rs` generates `C-Headers/sedsprintf.h` from the schema.
...
## Python binding
- `src/python_api.rs` defines the pyo3 module.
- `build.rs` generates `python-files/sedsprintf_rs/sedsprintf_rs.pyi` for type hints.
...
## Schema consistency
Both bindings are generated from `telemetry_config.json`. Any schema change requires regenerating and redeploying the bindings.
