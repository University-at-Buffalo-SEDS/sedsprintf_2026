...
The order of items in `telemetry_config.json` defines the enum discriminants. Those discriminants are sent on the wire and used in endpoint bitmaps. Reordering entries without updating every deployed system will break decode compatibility.
